...
@section('topbar-actions')
  {{-- Bouton export Excel --}}
  <a href="{{ route('secretaire.enseignants.export-excel') }}"
  ...
  <div style="padding:12px 20px;border-bottom:1px solid var(--border);display:flex;gap:10px;flex-wrap:wrap;">
    <div style="position:relative;flex:1;min-width:160px;">
      ...
      <input type="text" id="search" oninput="filterTable()" placeholder="Rechercher un enseignant…"
        style="width:100%;padding:8px 12px 8px 34px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;background:#FAFAFA;outline:none;font-family:inherit;color:var(--navy);"
        onfocus="this.style.borderColor='#00C07F'" onblur="this.style.borderColor='var(--border)'">
    </div>
    <select id="filterSt" onchange="filterTable()"
      style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;background:#FAFAFA;outline:none;cursor:pointer;font-family:inherit;color:var(--navy);">
        ...
        <tr data-statut="{{ strtolower($ens->statut?->lib_stat ?? '') }}" data-nom="{{ strtolower($ens->nom_complet) }}">
          <td>
            <div style="display:flex;align-items:center;gap:10px;">
              ...
              <div>
                ...
                <div style="font-size:11px;color:var(--muted);">{{ $ens->utilisateur?->email ?? 'Pas de compte' }}</div>
              </div>
            </div>
          </td>
          ...
          <td>
            <div style="display:flex;gap:6px;">
              ...
              <form method="POST" action="{{ route('secretaire.enseignants.destroy',$ens) }}" onsubmit="return confirm('Supprimer cet enseignant ?')">
                @csrf @method('DELETE')
...
function filterTable(){
  ...
  document.querySelectorAll('#tbl tbody tr[data-nom]').forEach(function(r){
    var ok=(!s||r.dataset.nom.includes(s))&&(!st||r.dataset.statut.includes(st));
